/*
Builds Data/vocab/n4.js from a raw “Ready-made Contributed” Jisho-style dump.

Usage (from vocabfolder/):
  node tools/build_n4_from_dump.js --input tools/n4_dump.txt

Rules:
- Parses the dump into { kanji, readings, meanings }
- Generates romaji for kana readings and places romaji first in `readings`
- Removes entries that already exist in N3 (by kanji/reading match)
- Writes Data/vocab/n4.js
...
function parseDump(text) {
  ...
  for (const line of lines) {
    if (isLikelyHeaderLine(line)) {
      ...
      const readings = readingVariants.length > 0 ? buildReadingsWithRomaji(readingVariants) : (chosenKanji && isKanaLike(chosenKanji) ? buildReadingsWithRomaji([chosenKanji]) : []);
...
function writeN4(entries) {
  const out = [];
  out.push('window.kanjiData = window.kanjiData || {};');
  out.push('window.kanjiData.N4 = [');
...
function main() {
  if (!fs.existsSync(inputPath)) {
    ...
    console.error('Create it and paste your N4 dump into it, then re-run.');
    process.exit(1);
  }
