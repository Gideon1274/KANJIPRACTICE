/*
Builds Data/contributed.js from a raw “Ready-made Contributed” Jisho-style dump.

Usage (from vocabfolder/):
  node tools/build_contributed_from_dump.js --input tools/contributed_dump.txt

Rules:
- Parses the dump into { kanji, readings, meanings }
- Generates romaji for kana readings and places romaji first in `readings`
- NO overlap filtering (everything stays)
- Writes Data/contributed.js as window.kanjiData.CONTRIBUTED
...
const inputPath = path.resolve(ROOT, getArg('--input') || 'tools/contributed_dump.txt');
const outPath = path.resolve(ROOT, 'Data/vocab/contributed.js');
...
function writeContributed(entries) {
  const out = [];
  out.push('window.kanjiData = window.kanjiData || {}; window.kanjiData.CONTRIBUTED = [');
...
function main() {
  if (!fs.existsSync(inputPath)) {
    ...
    console.error('Create it and paste your Contributed dump into it, then re-run.');
    process.exit(1);
  }
