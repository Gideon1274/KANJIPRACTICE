/*
Builds Data/vocab/n3.js from a raw “Ready-made Contributed” Jisho-style dump.

Usage (from vocabfolder/):
  node tools/build_n3_from_dump.js --input tools/n3_dump.txt

Rules:
- Parses the dump into { kanji, readings, meanings }
- Generates romaji for kana readings and places romaji first in `readings`
- DOES NOT dedupe against other JLPT levels (N3 is the baseline)
- Writes Data/vocab/n3.js
...
const inputPath = path.resolve(ROOT, getArg('--input') || 'tools/n3_dump.txt');
const outPath = path.resolve(ROOT, 'Data/vocab/n3.js');
...
function main() {
  if (!fs.existsSync(inputPath)) {
    ...
    console.error('Create it and paste your N3 dump into it, then re-run.');
    process.exit(1);
  }
