/*
Builds Data/vocab/n1.js from a raw “Ready-made Contributed” Jisho-style dump.

Usage (from vocabfolder/):
  node tools/build_n1_from_dump.js --input tools/n1_dump.txt

What it does:
- Parses the dump into { kanji, readings, meanings }
- Prioritizes a kanji form for `kanji` when present
- Removes entries that already exist in N2/N3 (by kanji and reading match)
- Writes Data/vocab/n1.js

Notes:
- This is a heuristic parser for the text dump format (not an official API).
- It intentionally ignores metadata lines like "esp.", "usu.", "oft.", "See ...・1", etc.
*/
...
const inputPath = path.resolve(ROOT, getArg('--input') || 'tools/n1_dump.txt');

const n2Path = path.resolve(ROOT, 'Data/vocab/n2.js');
const n3Path = path.resolve(ROOT, 'Data/vocab/n3.js');
const outPath = path.resolve(ROOT, 'Data/vocab/n1.js');
...
function loadKeysFromDataFile(jsPath) {
  ...
  // Ignore whole-line comments so commented-out entries don't count.
  const src = noBlock
    .split(/\r?\n/)
...
function parseDump(text) {
  ...
  for (const line of lines) {
    if (isLikelyHeaderLine(line)) {
      ...
      // Readings should be kana; never fall back to kanji.
      const readings = readingVariants.length > 0 ? buildReadingsWithRomaji(readingVariants) : (chosenKanji && isKanaLike(chosenKanji) ? [chosenKanji] : []);
...
function main() {
  if (!fs.existsSync(inputPath)) {
    ...
    console.error('Create it and paste your N1 dump into it, then re-run.');
    process.exit(1);
  }
